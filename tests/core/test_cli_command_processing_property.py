...
class TestCLICommandProcessingProperty:
    ...
    @given(
        ...
        input_source=st.sampled_from([InputSource.ARGS, InputSource.STDIN, InputSource.FILE]),
    ...
    def test_cli_command_processing_property(self, command_name: str, input_content: str, 
                                           input_source: InputSource, options: Dict[str, Any]):
        """
        **Feature: python-devtui-toolkit, Property 21: CLI command processing**
        **Validates: Requirements 6.2**
        ...
        assert result.success is True, f"Command {command_name} should succeed for valid input"
        assert result.output is not None, "Successful processing should produce output"
        assert result.error_message is None, "Successful processing should not have error message"
        
        # Verify output contains processed input
        assert isinstance(result.output, str), "Output should be a string"
        assert "Processed:" in result.output, "Output should indicate processing occurred"
        assert input_content in result.output, "Output should contain original input content"
        
        # Verify metadata is populated correctly
        assert 'input_length' in result.metadata, "Metadata should contain input length"
        assert result.metadata['input_length'] == len(input_content), "Input length should be preserved"
        assert result.metadata['source'] == input_source.value, "Input source should be preserved"
        assert result.metadata['options_count'] == len(options), "Options count should be preserved"
    ...
    def test_invalid_command_handling_property(self, invalid_command: str, 
                                             input_content: str, input_source: InputSource):
        """
        ...
        assert result.error_message is not None, "Failed processing should have error message"
        assert "Unknown command" in result.error_message, "Error should indicate unknown command"
        assert invalid_command in result.error_message, "Error should mention the invalid command"
    
    @given(
        command_name=st.sampled_from(["test", "test2", "test3"]),
        input_source=st.sampled_from([InputSource.ARGS, InputSource.STDIN, InputSource.FILE])
    )
    def test_empty_input_handling_property(self, command_name: str, input_source: InputSource):
        """
        ...
        for empty_content in empty_inputs:
            ...
            # Assert - Property verification
            assert result.success is False, f"Empty input '{repr(empty_content)}' should fail validation"
            ...
            assert "Invalid input" in result.error_message, "Error should indicate invalid input"
    ...
    def test_invalid_options_handling_property(self, command_name: str, input_content: str, 
        ...
        # Act
        validation_result = self.router.validate_command_options(command_name, invalid_options)
        
        # Assert - Property verification
        assert validation_result.success is False, "Invalid options should fail validation"
        assert validation_result.error_message is not None, "Failed validation should have error message"
        assert "Unsupported options" in validation_result.error_message, "Error should indicate unsupported options"
        ...
        assert error_mentions_invalid_option, "Error should mention at least one invalid option"
    ...
    def test_valid_options_handling_property(self, command_name: str, input_content: str, 
        ...
        # Act
        validation_result = self.router.validate_command_options(command_name, valid_options)
        processing_result = self.router.route_command(command_name, input_data, valid_options)
        
        # Assert - Property verification
        assert validation_result.success is True, "Valid options should pass validation"
        assert validation_result.error_message is None, "Successful validation should not have error"
        
        assert processing_result.success is True, "Processing with valid options should succeed"
        assert processing_result.output is not None, "Successful processing should produce output"
        assert processing_result.metadata['options_count'] == len(valid_options), "Options should be passed correctly"
    ...
    def test_command_help_property(self):
        ...
        for command_name in commands:
            ...
            # Assert - Property verification
            assert help_text is not None, f"Help should be available for command '{command_name}'"
            assert isinstance(help_text, str), "Help text should be a string"
            assert len(help_text.strip()) > 0, f"Help text should not be empty for command '{command_name}'"
            assert command_name in help_text or "Help for" in help_text, "Help should reference the command"
    ...
    def test_general_help_property(self):
        ...
        commands = self.registry.list_commands()
        for command_name in commands:
            assert command_name in general_help, f"General help should mention command '{command_name}'"