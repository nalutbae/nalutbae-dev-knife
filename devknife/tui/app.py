...
class UtilitySelectionScreen(Screen):
    ...
    def action_select_focused(self) -> None:
        ...
        if focused and hasattr(focused, 'id') and focused.id and focused.id.startswith("cmd-"):
...
class UtilityFormScreen(Screen):
        ...
        if utility_class:
            try:
                ...
                if supported_options:
                    ...
                    container.mount(Label("No additional options available for this utility."))
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "run-btn":
            ...
            self._load_file()
        elif event.button.id and event.button.id.startswith("opt-") and "checkbox-button" in event.button.classes:
        ...
        with Container(id="result-container"):
            ...
            if self.result.success:
                ...
                with Collapsible(title="Output", collapsed=False):
                    output_text = str(self.result.output) if self.result.output is not None else ""
                    yield TextArea(
                        output_text,
                        read_only=True,
                        id="output-text"
    ...
    def action_copy(self) -> None:
        ...
        if self.result.success and self.result.output is not None:
            try:
                ...
                self.notify("Clipboard functionality not available. Output is displayed above for manual copying.")
            except Exception as e:
