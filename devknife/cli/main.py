...
from devknife.utils.data_format_utility import (
    JSONFormatter, JSONToYAMLConverter, XMLFormatter, JSONToPythonClassGenerator,
    CSVToMarkdownConverter, TSVToMarkdownConverter, CSVToJSONConverter
)
from devknife.utils.developer_utility import (
    UUIDGenerator, UUIDDecoder, IBANValidator, PasswordGenerator
)
from devknife.utils.math_utility import (
    NumberBaseConverter, HashGenerator, TimestampConverter
)
from devknife.utils.web_utility import (
    GraphQLFormatter, CSSFormatter, CSSMinifier, URLExtractor
)
...
@main.command()
@click.argument('json_text', required=False)
@click.option('--class-name', default='GeneratedClass', help='생성할 클래스 이름 (기본값: GeneratedClass)')
@click.option('--file', '-f', type=click.Path(exists=True), help='파일에서 입력을 읽습니다')
...
@main.command()
@click.argument('csv_text', required=False)
@click.option('--no-header', is_flag=True, help='모든 행을 데이터로 처리 (배열의 배열 생성)')
...
@click.option('--version', type=click.Choice(['1', '4']), default='4', help='UUID 버전 (기본값: 4)')
...
@main.command()
@click.option('--length', type=int, default=16, help='패스워드 길이 (기본값: 16, 최소: 4, 최대: 256)')
...
@click.option('--no-ambiguous', is_flag=True, help='혼동하기 쉬운 문자 제외 (0, O, l, 1, |)')
...
@click.option('--from', 'from_base', type=click.Choice(['auto', 'binary', 'octal', 'decimal', 'hex']), 
              default='auto', help='소스 진법 (기본값: auto)')
@click.option('--to', 'to_base', type=click.Choice(['binary', 'octal', 'decimal', 'hex', 'all']), 
              default='all', help='대상 진법 (기본값: all)')
@click.option('--file', '-f', type=click.Path(exists=True), help='파일에서 입력을 읽습니다')
...
@main.command()
@click.argument('text', required=False)
@click.option('--algorithm', type=click.Choice(['md5', 'sha1', 'sha256', 'all']), 
              default='all', help='해시 알고리즘 (기본값: all)')
@click.option('--file', '-f', type=click.Path(exists=True), help='파일에서 입력을 읽습니다')
...
@click.option('--format', 'output_format', type=click.Choice(['iso', 'readable']), 
              default='iso', help='출력 형식 (기본값: iso)')
