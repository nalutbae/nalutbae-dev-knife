    ...
    def _parse_number(self, content: str, from_base: str) -> tuple[int, str]:
        ...
        if from_base == 'auto':
            ...
            if content.startswith('0b') or content.startswith('0B'):
                ...
                return int(content, 2), 'binary'
            elif re.match(r'^[0-7]+$', content) and len(content) > 1 and content[0] == '0':
                # Looks like octal (starts with 0 and only contains 0-7)
                return int(content, 8), 'octal'
            elif re.match(r'^[0-9a-fA-F]+$', content) and any(c in content.lower() for c in 'abcdef'):
                # Contains hex digits
                return int(content, 16), 'hexadecimal'
            else:
                # Assume decimal
            ...
            if from_base in ['hex', 'hexadecimal'] and (content.startswith('0x') or content.startswith('0X')):
                clean_content = content[2:]
            elif from_base == 'binary' and (content.startswith('0b') or content.startswith('0B')):
                clean_content = content[2:]
            elif from_base == 'octal' and (content.startswith('0o') or content.startswith('0O')):
                clean_content = content[2:]
...
class HashGenerator(UtilityModule):
    ...
    def _generate_hash(self, content: str, algorithm: str) -> str:
        ...
        if algorithm == 'md5':
            ...
            raise ValueError(f"Unsupported hash algorithm: {algorithm}. Supported: md5, sha1, sha256")
...
class TimestampConverter(UtilityModule):
    ...
    def _convert_from_timestamp(self, content: str, output_format: str, use_utc: bool) -> ProcessingResult:
        """
        Convert Unix timestamp to human-readable format.
