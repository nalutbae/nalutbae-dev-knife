...
class UnifiedErrorHandler:
    ...
    def format_error_for_tui(self, result: ProcessingResult) -> dict:
        ...
        return {
            ...
            'suggestions': result.metadata.get('suggestions', []) if result.metadata else [],
            'severity': result.metadata.get('severity', 'error') if result.metadata else 'error',
            'details': result.metadata.get('error_details', '') if result.metadata else ''
        }
